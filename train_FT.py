...
def train_and_test_model(cfg):
    ...
    train_loader = DataLoader(FT(cfg.data_path, 'train', crop_train=True), batch_size=cfg.batch_size, shuffle=True)
    test_loader = DataLoader(FT(cfg.data_path, 'test', crop_train=True), batch_size=1, shuffle=False)
    ...
    for epoch in range(cfg.epochs):
        ...
        for batch_idx, (focal_stack, aif_gt,  depth_gt) in pbar:
            optimizer.zero_grad()

            focal_stack, aif_gt, depth_gt = focal_stack.to(cfg.device), aif_gt.to(cfg.device) , depth_gt.to(cfg.device)
...
def save_test_depths(model, test_loader, epoch, cfg):
    ...
    with torch.no_grad():
        for i, (focal_stack, aif_gt, depth_gt) in enumerate(test_loader):
            focal_stack, aif_gt,  depth_gt = focal_stack.to(cfg.device), aif_gt.to(cfg.device), depth_gt.to(cfg.device)
